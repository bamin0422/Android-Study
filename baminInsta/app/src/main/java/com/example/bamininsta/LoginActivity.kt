...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var gso = GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
            .requestIdToken("554455956960-0cdt14cf8ahpu1sb1466qq1sajhufa2e.apps.googleusercontent.com")
            .requestEmail()
            .build()
    ...
    fun handleFacebookAccessToken(token : AccessToken?){
        var credentail = FacebookAuthProvider.getCredential(token?.token!!)
        auth?.signInWithCredential(credentail)?.addOnCompleteListener{ task ->
    ...
    fun signinAndSignup(){
        auth?.createUserWithEmailAndPassword(email_edittext.text.toString(), password_edittext.text.toString())?.addOnCompleteListener {
            task ->
            if(task.isSuccessful){
                Toast.makeText(this, email_edittext.text.toString()+"로 가입완료", Toast.LENGTH_LONG).show()
